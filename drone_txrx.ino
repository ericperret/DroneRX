/*
 * ══════════════════════════════════════════════════════════════
 *  AH & EPERRET — DRONE RX V3.3
 *  M5Stack CoreS3 — M5Unified
 *
 *  V3.3 Waterfall :
 *    - Unknown beacons plotés en cyan sombre (watchdog)
 *    - Fond RSSI par canal (vert/jaune/orange/rouge sombre)
 *    - Zéro impact sur le flux fonctionnel
 *
 *  Corrections V3.2 :
 *    - Country code FR pour canal 13 (fix SSID ESP_XXXX)
 *    - OUI length checks (trames tronquées)
 *    - Touch debounce (attente relâchement)
 *    - Promiscuous uniquement dans startSniffMode()
 *    - Page HTML : fermer avant d'ouvrir (USB + WS)
 *
 *  Architecture :
 *    USB Serial = mode optimal (radio 100% réception passive)
 *    WiFi AP    = fallback automatique (canal 13, multi-clients)
 *
...
// ── WiFi Access Point ──
#define AP_SSID         "DroneRX"
#define AP_PASS         "dronesniff"
...
// ── Scan canaux ──
#define DWELL_BASE_MS   200
#define DWELL_JITTER_MS 30
...
const char* SCAN_MODE_NAMES[] = { "DETECTION", "MIXTE", "TRACKING" };

static const uint8_t SEQ_DETECT[] = { 6, 0, 1, 0, 11, 0 };
#define SEQ_DETECT_LEN 6

static const uint8_t SEQ_MIXTE[] = { 6, 1, 6, 11, 6, 0 };
#define SEQ_MIXTE_LEN 6

static const uint8_t ROTATE_CH[] = { 2, 3, 4, 5, 7, 8, 9, 10, 12, 13, 14 };
#define NUM_ROTATE 11
...
#define BTN_PWR_X  10          // PWR OFF : x 10–154
#define BTN_PWR_W  144
#define BTN_MODE_X 166         // MODE : x 166–310
#define BTN_MODE_W 144
...
uint32_t  lastUSBHeartbeat = 0;
uint32_t  lastUSBCheck     = 0;
...
typedef struct {
  ...
  bool     idValid;
  double   lat, lon, alt;      // position courante (centre carte USB)
  ...
  while (Serial.available()) {
    ...
    if (c == '\n' || c == '\r') {
      if (serialInLen > 0) {
        ...
        if (strncmp(serialInBuf, "HB", 2) == 0) {
          lastUSBHeartbeat = millis();
        }
        // ── V3.3 TX : reception GPS depuis la page ──
        else if (serialInBuf[0] == '{') {
...
void enablePromiscuous() {
  // ── V3.3 : country JP pour canal 14 (aussi en mode USB) ──
...
void checkTransport() {
  if (millis() - lastUSBCheck < USB_CHECK_MS) return;
  lastUSBCheck = millis();

  bool usbAlive = (millis() - lastUSBHeartbeat < USB_TIMEOUT_MS);

  if (usbAlive && activeTransport != XPORT_USB) {
    Serial.println("[XPORT] USB actif → mode optimal");
    activeTransport = XPORT_USB;
    stopWiFiAP();

    // ── FIX V3.2 : clean WiFi state before STA ──
    WiFi.mode(WIFI_OFF);
    delay(100);
    WiFi.mode(WIFI_STA);
    WiFi.disconnect();

    // ── FIX V3.2 : promiscuous seulement si on est en sniff ──
    if (appState == STATE_SNIFF) {
      enablePromiscuous();
    }

    M5.Speaker.tone(1500, 50); delay(60);
    M5.Speaker.tone(2000, 50); delay(60);
    M5.Speaker.stop();
    updateTransportDisplay();
  }
  else if (!usbAlive && activeTransport == XPORT_USB) {
    Serial.println("[XPORT] USB perdu → fallback WiFi");
    esp_wifi_set_promiscuous(false);
    startWiFiAP();
    ...
    M5.Speaker.tone(500, 100); delay(120);
    M5.Speaker.stop();
    updateTransportDisplay();
  }
  else if (!usbAlive && activeTransport == XPORT_NONE) {
    // ── FIX V3.2 : au boot, juste l'AP WiFi, PAS de promiscuous ──
    startWiFiAP();
  }
}
...
void handleHTTP() {
  ...
  uint32_t pageLen = strlen_P(PAGE_HTML);
  ...
  client.stop();
  Serial.println("[HTTP] Page servie");
}
...
void wifi_sniffer_cb(void* buf, wifi_promiscuous_pkt_type_t type) {
  ...
  // ── V3.3 WATERFALL : RSSI par canal, maj par TOUTE beacon ──
...
uint8_t nextChannel() {
  if (scanMode == MODE_TRACKING && selectedDrone >= 0 && selectedDrone < droneCount)
    return droneList[selectedDrone].channel;

  const uint8_t* seq;
  uint8_t seqLen;
  if (scanMode == MODE_DETECTION) { seq = SEQ_DETECT; seqLen = SEQ_DETECT_LEN; }
  else { seq = SEQ_MIXTE; seqLen = SEQ_MIXTE_LEN; }

  uint8_t ch = seq[seqIndex];
  seqIndex = (seqIndex + 1) % seqLen;

  if (ch == 0) {
    ch = ROTATE_CH[rotateIndex];
    rotateIndex = (rotateIndex + 1) % NUM_ROTATE;
  }
  return ch;
}
...
void switchChannel() {
  ...
  esp_wifi_set_channel(currentChannel, WIFI_SECOND_CHAN_NONE);
  // ── V3.3 : canal scanné mais vide = calme = vert ──
  if (currentChannel >= 1 && currentChannel <= 14 && channelRSSI[currentChannel] == -100) {
    channelRSSI[currentChannel] = -95;
  }
}
...
void drawSplash() {
  ...
  M5.Display.println("DRONE_RX V3.3");
...
void drawScanScreen() {
  ...
  M5.Display.println("DRONE RX V3.3");
...
void drawHelpScreen() {
  ...
  M5.Display.println("1. WiFi > DroneRX / dronesniff");

  M5.Display.setTextColor(TFT_YELLOW, C_BG);
  M5.Display.setCursor(6, 52);
  M5.Display.println("2. Navigateur > 192.168.4.1");

  M5.Display.setTextColor(TFT_GREEN, C_BG);
  M5.Display.setCursor(6, 72);
  M5.Display.println("3. Cliquer [Sauvegarder]");

  M5.Display.setTextColor(TFT_YELLOW, C_BG);
  M5.Display.setCursor(6, 92);
  M5.Display.println("4. Quitter WiFi DroneRX");
  M5.Display.setTextColor(TFT_DARKGREY, C_BG);
  M5.Display.setCursor(6, 104);
  M5.Display.println("   Rejoindre votre WiFi/4G");
  ...
  M5.Display.println("   (carte live + USB dispo)");

  M5.Display.setTextColor(TFT_YELLOW, C_BG);
  M5.Display.setCursor(6, 156);
  M5.Display.println("6. Brancher USB > [USB Serial]");
  M5.Display.setTextColor(TFT_DARKGREY, C_BG);
  M5.Display.setCursor(6, 168);
  M5.Display.println("   Radio 100% RX, zero WiFi");

  M5.Display.setTextColor(TFT_RED, C_BG);
  M5.Display.setCursor(6, 192);
  M5.Display.println("Sans USB: WS auto via WiFi DroneRX");
...
void drawModeSelect() {
  ...
  M5.Display.println("1,6,11=50% autres=50%");
  ...
  M5.Display.println("MIXTE");
  M5.Display.setTextSize(1);
  M5.Display.setCursor(30, 100);
  M5.Display.println("ch6 boost + 1,11 + scan");
  ...
  M5.Display.println("Lock 1 drone");
...
void drawSniffScreen() {
  ...
  if (scanMode == MODE_TRACKING && selectedDrone >= 0) {
    M5.Display.setTextSize(1);
    M5.Display.printf(" CH%d", droneList[selectedDrone].channel);
  }
...
void drawTransportLine() {
  ...
  M5.Display.setTextColor(TFT_DARKGREY, C_BG);
  M5.Display.setCursor(10, 44);
  if (activeTransport == XPORT_USB)
    M5.Display.println("USB Serial - radio 100% RX");
  else
    M5.Display.printf("WiFi:%s ws://192.168.4.1:%d", AP_SSID, WS_PORT);
}
...
void drawTouchButtons() {
  ...
  M5.Display.setTextSize(1);
  M5.Display.setTextColor(TFT_RED, 0x4000);
  M5.Display.setCursor(BTN_PWR_X + 42, BTN_Y + 4);
  ...
  M5.Display.setCursor(BTN_MODE_X + 38, BTN_Y + 4);
...
void drawBottomBar() {
  ...
  char buf[54];
  const char* xp = (activeTransport == XPORT_USB) ? "USB" : "WiFi";
...
void startSniffMode() {
  ...
  Serial.printf("\n[RX] Mode %s actif [%s]\n\n",
    ...
    activeTransport == XPORT_USB ? "USB" : "WiFi");
...
void drawTxScreen() {
  ...
  M5.Display.println("Position = centre carte USB");
...
void setup() {
  ...
  Serial.println();
  Serial.println("===========================================");
  Serial.println(" AH & EPERRET — Drone RX V3.3");
  Serial.println(" USB primary / WiFi fallback");
  Serial.println(" Waterfall SDR + UNK + RSSI bg");
  Serial.println("===========================================");
  Serial.println();
...
void loop() {
  ...
  if (appState == STATE_SCAN) {
    ...
    if (t.isPressed()) {
      ...
      while (true) { M5.update(); auto r = M5.Touch.getDetail(); if (!r.isPressed()) break; delay(20); }
      checkTransport();
      appState = STATE_HELP;       // ── V3.3 : HELP avant MODE ──
      drawHelpScreen();
    }
    delay(50);
    return;
      ...
      if (y >= 30 && y < 70) {
        ...
        if (droneCount == 0) {
          ...
          while (true) {
            ...
            if (tt.isPressed()) {
              int idx = (tt.y - 40) / 24;
              if (idx >= 0 && idx < droneCount) {
                selectedDrone = idx;
                M5.Speaker.tone(2000, 80); delay(100); M5.Speaker.stop();
                startSniffMode();
                return;
              }
            }
